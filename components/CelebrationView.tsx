...
import { CheckCircle, RefreshCw, Save } from 'react-feather'
  ...
  return (
    <div className="py-8 px-4">
      <div className="bg-[#F0FDF4] border-2 border-[#10B981] rounded-2xl p-6 text-center mb-6">
        <p className="text-2xl font-bold text-[#065F46] mb-2 inline-flex items-center gap-2"><CheckCircle size={24} /> Odpoveď:</p>
      ...
      <div className="flex flex-col gap-3">
        <button
          onClick={onSave}
          className="w-full py-4 rounded-2xl border-2 border-[#6D28D9] text-[#6D28D9] text-[18px] font-semibold hover:bg-[#EEF2FF] transition-colors inline-flex items-center justify-center gap-2"
        >
          <Save size={18} /> Uložiť do histórie
        </button>
        <button
          onClick={onReset}
          className="w-full py-4 rounded-2xl bg-[#F59E0B] text-white text-[18px] font-semibold hover:bg-amber-500 transition-colors active:scale-95 inline-flex items-center justify-center gap-2"
        >
          <RefreshCw size={18} /> Nový príklad
        </button>
      </div>
    </div>
  )
